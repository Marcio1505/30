...
import { resendConfirmationEmail } from '../../../services/apis/auth.api';
...
const ResendConfirmationEmail = () => {
  ...
  const onSubmit = async (values) => {
    const resp = await resendConfirmationEmail({ data: values });
    if (resp?.email) {
      setEmailSent(resp?.email);
    }
  };
                  ...
                  {emailSent ? (
                    <>
                      <h5 className="mb-2 px-md-2">
                        Foi enviado para um novo email de confirmação para o
                        email: {emailSent}
                      </h5>
                    ...
                    <h5 className="mb-0 px-md-2">
                      Informe seu CPF para receber um novo email de confirmação
                      da sua conta iuli
                    </h5>
                  )}
                </div>
