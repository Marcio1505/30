...
class SideMenuContent extends React.Component {
  ...
  handleGroupClick = (id, parent = null, type = '') => {
    ...
    if (type === 'collapse') {
      ...
      if (!open_group.includes(id)) {
        ...
        const temp = open_group.filter(
          (obj) => active_group.indexOf(obj) === -1
        );
  ...
  componentDidUpdate(prevProps, prevState) {
    // console.log('activeItemState', this.props.activeItemState);
  ...
  render() {
    ...
    const menuItems = navigationConfig.map(item => {
      ...
      const renderItem = (
        <li
          className={classnames('nav-item', {
            ...
            active:
              ...
              (item.children &&
                item.children.includes(this.props.activeItemState)),
        ...
        >
          <PermissionGate permissions={item.permissioniuli}>
            <CustomAnchorTag
              to={
                item.filterBase
                  ? item.filterBase
              ...
              key={item.id}
              onClick={(e) =>
                item.type === 'collapse' ? e.preventDefault() : ''
              }
